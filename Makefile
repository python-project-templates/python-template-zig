...
lint:  ## run python linter with ruff
	python -m ruff check python_template_zig
	python -m ruff format --check python_template_zig

# Alias
lints: lint

fix:  ## fix python formatting with ruff
	python -m ruff check --fix python_template_zig
	python -m ruff format python_template_zig
...
annotate:  ## run python type annotation checks with mypy
	python -m mypy ./python_template_zig
...
test:  ## run python tests
	python -m pytest -v python_template_zig/tests

coverage:  ## run tests and collect test coverage
	python -m pytest -v python_template_zig/tests --cov=python_template_zig --cov-report term-missing --cov-report xml
